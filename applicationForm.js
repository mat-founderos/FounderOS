...
$(document).ready(function () { 
  ...
  async function a() {
    c();
    try {
      if (!(await fetch("https://founderos.app.n8n.cloud/webhook-test/b82aaa3a-642a-41ce-a02f-22e8d93bbd11", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({type: "partial", formData: currentFormData, step: currentStep, timestamp: (new Date).toISOString()})})).ok) {
  ...
  window.addEventListener("beforeunload", async function (t) {
    c();
    try {
      await fetch("https://founderos.app.n8n.cloud/webhook/webhook/partial-lead", {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({type: "partial", formData: currentFormData, step: currentStep, timestamp: (new Date).toISOString()}), keepalive: true});
    } catch (t) {
